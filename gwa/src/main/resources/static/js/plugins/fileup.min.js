(function ($) {
    var default_options = {
        ...
        timeout: null,
        autostart: false,
        templateFile: '<div id="fileup-[INPUT_ID]-[FILE_NUM]" class="fileup-file [TYPE]"><div class="fileup-preview"><img src="[PREVIEW_SRC]" alt="[NAME]"/></div><div class="fileup-container"><div class="fileup-description"><span class="fileup-name">[NAME]</span> (<span class="fileup-size">[SIZE_HUMAN]</span>)</div><div class="fileup-controls"><span class="fileup-remove" onclick="$.fileup(\'[INPUT_ID]\', \'remove\', \'[FILE_NUM]\');" title="[REMOVE]"></span><span class="fileup-upload" onclick="$.fileup(\'[INPUT_ID]\', \'upload\', \'[FILE_NUM]\');">[UPLOAD]</span><span class="fileup-abort" onclick="$.fileup(\'[INPUT_ID]\', \'abort\', \'[FILE_NUM]\');" style="display:none">[ABORT]</span></div><div class="fileup-result"></div><div class="fileup-progress"><div class="fileup-progress-bar"></div></div></div><div class="fileup-clear"></div></div>',
        ...
        onStartSystem: function (file_number, file) {
            var options = this.fileup.options, $file = $('#fileup-' + options.inputID + '-' + file_number);
        ...
        onErrorSystem: function (event, file, file_number) {
            ...
            switch (event) {
                case'files_limit':
                    ...
                    break;
                case'size_limit':
                    var size = formatHuman(options.sizeLimit), message = i18n[options.lang].errorSizeLimit;
                ...
                case'file_type':
                    ...
                    break;
                case'file_duplicate':
                    var message = i18n[options.lang].errorFileDuplicate;
                    alert(message.replace(/%fileName%/g, file.name));
                    break;
                case'bad_status':
                case'error_load':
        ...
        addEvent: function (input, name, callback) {
            ...
            var result = true, event_result;
            if (input.fileup.events[name]) for (var i = 0; i < input.fileup.events[name].length; i++) if (typeof input.fileup.events[name][i] === 'function') if (args) {
                event_result = input.fileup.events[name][i].apply(input, args);
                if (result && event_result === false) result = false
            } else {
        ...
        input: {}, select: function (callback) {
        ...
        ru: {
            ...
            errorFileDuplicate: 'Файл "%fileName%" является некорректным',
        ...
        en: {
            ...
            errorFileDuplicate: 'File "%fileName%" is already selected',
    ...
    function init(param1) {
        var options = $.extend(false, default_options, param1), input = document.getElementById(options.inputID);
        if (input && input.type === 'file') {
            if (options.dropzoneID) {
                ...
                if (dropZone) {
                    ...
                    dropZone.fileup = {input: input}
                }
            }
            ;input.fileup = {files: {}, nextID: 0, options: options, events: {}};
            ...
            if (options.files.length > 0) for (var i = 0; i < options.files.length; i++) {
                var tpl = options.templateFile;
                tpl = tpl.replace(/\[INPUT_ID\]/g, options.inputID);
                tpl = tpl.replace(/\[FILE_NUM\]/g, options.files[i].id);
                ...
                input.fileup.files[options.files[i].id] = {
                    xhr: null,
                    file: options.files[i],
                    file_number: options.files[i].id,
                    status: 'loaded'
                };
                ...
                if (options.files[i].downloadUrl) {
                    ...
                    if ($name[0]) $name.replaceWith('<a href="' + options.files[i].downloadUrl + '" class="fileup-name" download="' + options.files[i].name + '">' + options.files[i].name + '</a>')
                }
                ;
                // input.fileup.nextID++
            }
    ...
    function dropFiles(event) {
        event.preventDefault();
        event.stopPropagation();
        var input = event.target.fileup.input, options = input.fileup.options,
            ...
            for (var i = 0; i < files.length; i++) {
                ...
                if (typeof input.accept === 'string') {
                    ...
                    if (accept && /[^\w]+/.test(accept)) {
                        var is_accept = false, types = accept.split(',');
                        if (types.length > 0) for (var t = 0; t < types.length; t++) {
                ...
                ;
                if (events.callEvent(input, 'select', [file]) === false) continue;
    ...
    function appendFiles() {
        if (this.files.length) {
            var options = this.fileup.options, multiple = $(this).is("[multiple]");
            if (!multiple) if (events.callEvent(this, 'remove', ['*', '1', this.files[0]])) {
            ...
            for (var i = 0; i < this.files.length; i++) {
                ...
                var list_file_num = Object.keys(this.fileup.files);
                var listFile = this.fileup.files;
                var isDuplacate = false;
                for (var j = 0; j < list_file_num.length; j++) {
                    if (file.name === listFile[list_file_num[j]].file.name){
                        isDuplacate = true;
                        break;
                    }
                }
                if (isDuplacate){
                    events.callEvent(this, 'error', ['file_duplicate', file]);
                    continue
                }
                ;
                ...
                ;
                if (events.callEvent(this, 'select', [file]) === false) continue;
    ...
    function appendFile(file, input) {
        ...
        ;
        var list_file_num = Object.keys(input.fileup.files);
        for (var i = 0; i < list_file_num.length; i++) {
            if (input.fileup.nextID == list_file_num[i]){
                input.fileup.nextID++;
            }
        }
        var tpl = options.templateFile;
        ...
        tpl = tpl.replace(/\[REMOVE\]/g, i18n[options.lang].remove);
        tpl = tpl.replace(/\[ABORT\]/g, i18n[options.lang].abort);
        var fileContainer = {xhr: xhr, file: file, file_number: input.fileup.nextID, status: 'stand_by'};
        input.fileup.files[input.fileup.nextID] = fileContainer;
        list_file_num = Object.keys(input.fileup.files);
        input.fileup.nextID++;
        if (file.type === 'image/gif' || file.type === 'image/png' || file.type === 'image/jpg' || file.type === 'image/jpeg') {
            if (typeof FileReader !== 'undefined') {
                ...
                reader.onload = function (ProgressEvent) {
                    ...
                    if (options.autostart) uploadFile(input, fileContainer)
                };
    ...
    function formatHuman(bytes) {
        ...
        } else if (bytes >= 0) result = bytes + ' bytes';
        return result
    }

    function uploadFile(input, fileContainer) {
        var options = input.fileup.options, file_number = fileContainer.file_number, file = fileContainer.file,
            xhr = fileContainer.xhr;
        ...
        xhr.onload = function () {
            ...
            } else events.callEvent(input, 'error', ['bad_status', file, file_number, xhr.responseText])
        };
        ...
        if (window.FormData !== undefined) {
            ...
            formData.append(options.fieldName, file, options.prefixName + file_number + '.' + file.type.split("/")[1]);
            if (Object.keys(options.extraFields).length) $.each(options.extraFields, function (name, value) {
                formData.append(name, value)
            });
            return xhr.send(formData)
        } else return xhr.send(file)
    };

    var methods = {
        upload: function (inputID, file_number) {
            var input = document.getElementById(inputID);
            if (input && input.type === 'file' && typeof input.fileup === 'object') if (file_number === '*') {
                $.each(input.fileup.files, function (key, fileContainer) {
                    if (fileContainer.status === 'stand_by') uploadFile(input, fileContainer)
                })
            } else if (typeof input.fileup.files[file_number] === 'object') uploadFile(input, input.fileup.files[file_number])
        }, remove: function (inputID, file_number) {
            var input = document.getElementById(inputID);
            if (input && input.type === 'file' && typeof input.fileup === 'object') {
                var options = input.fileup.options, total = Object.keys(input.fileup.files).length;
                if (file_number === '*') {
                    if (events.callEvent(input, 'remove', ['*', total]) === false) return;
                    input.fileup.files = {};
                    $('[id^=fileup-' + inputID + '-]').fadeOut('fast', function () {
                        $(this).remove()
                    })
                    input.fileup.nextID = 0;
                } else if (typeof input.fileup.files[file_number] === 'object') {
                    if (events.callEvent(input, 'remove', [input.fileup.files[file_number], total, file_number]) === false) return;
            ...
            var input = document.getElementById(inputID);
            if (input && input.type === 'file' && typeof input.fileup === 'object') events.removeEvents(input, name)
            ...
            } else if (typeof input.fileup.files[file_number] === 'object') input.fileup.files[file_number].xhr.abort()
        }
    };
    ...
    $.fileup = function (param1, param2, param3) {
        ...
        } else $.error('Unknown method ' + param1)
    };
    $.fileup.updatePrefixName = function (inputID, prefixName) {
        var input = document.getElementById(inputID);
        input.fileup.options.prefixName = prefixName;
    };
